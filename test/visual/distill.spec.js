...
for (const theme of ["light", "dark"]) {
  test(`distill style remains stable after load (${theme})`, async ({
    page,
  }) => {
    await preparePage(page, theme);
    await page.goto("al-folio/blog/2021/distill/", {
  ...
  test(`distill visual parity against baseline (${theme})`, async ({
    page,
    context,
  }, testInfo) => {
    test.skip(
      !process.env.BASELINE_URL,
      "BASELINE_URL is not configured for visual parity checks.",
    );
    await preparePage(page, theme);
    const ratio = await compareWithBaseline(
      context,
      page,
